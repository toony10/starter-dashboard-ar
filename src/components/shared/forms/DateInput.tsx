...
const MERIDIEM_LABELS: Record<Meridiem, string> = {
  AM: "ص",
  PM: "م",
}
...
function formatDisplay(
  ...
  if (!withTime) return format(date, "PPP", { locale: ar })
  if (use12Hour) {
    return format(date, showSeconds ? "PPP p:ss" : "PPP p", { locale: ar })
  }
  return format(date, showSeconds ? "PPP HH:mm:ss" : "PPP HH:mm", { locale: ar })
}
          ...
          >
            ...
            { clearable && selectedDate && !disabled ? (
              <span
                ...
                className="ms-auto inline-flex size-6 shrink-0 items-center justify-center rounded-md text-muted-foreground hover:bg-muted hover:text-foreground"
                onClick={ handleClear }
                onKeyDown={ (event) => {
            ...
            <div className="space-y-3 border-t p-3">
              ...
              { !selectedDate ? (
                <p className="text-xs text-muted-foreground">
                  اختر التاريخ أولاً، ثم عدّل الوقت.
                </p>
              ) : null }
            </div>
          ) : null }
